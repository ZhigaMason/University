...
```{R warning=FALSE}
# TODO: formulate different question. This is not suited for this test
ct_test <- xtabs(formula=thousands_employed ~ geo + nace_cat, data.ct.test)
print(head(ct_test, 2))
chisq.test(ct_test)
```

P-value is much less than $\alpha = 0.05$, so we fail to reject the null hypothesis. Conclusion is that there is no relation between number of people employed in **SMALL_BUSINESS** and **LARGE_BUSINESS**.
