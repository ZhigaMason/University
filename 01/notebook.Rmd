...
data
```
Lastly we will delete columns **freq**, **unit**, **wstatus** and **TIME_PERIOD** as they are constant (*A* - for annual frequency, *THS* - for one thousand unit, *EMP* - manually chosen wstatus, and *2022-01-01*), also **nace_r2** feature will be removed, becaused it was already transformed. Remaining **geo** and **nace_cat** features will be turned into factors and **values** will be renamed into **thousands_employed**.
...
each entry in contingency table is number of thousands of people in regio **geo** employed in **nace_cat** category. The last column and the last row are marginal sums, they are the sums of their respective row or column.
...
```
Distribution look almost the same, so we can say that they are the same.
